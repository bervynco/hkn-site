...
@Component({
  ...
  styleUrls: ['./celebirty.component.css']  // Fixed plural form
})
export class CelebirtyComponent implements OnInit {
    ...
    this.httpArticle.getArticle().subscribe({
      next: (response) => {
        if (response && Array.isArray(response.posts)) {
          // Filter posts to include only those with type 'Celebrity'
      ...
      error: (error) => {
        ...
        alert('Failed to load celebrity articles. Please try again later.');
      },
    });
  }
  ...
  getPost(type: string, slug: string, article: any) {
    ...
    this.httpArticle.getsinglepost(type, slug).subscribe(result => {
      ...
      if (isPlatformBrowser(this.platformId)) {
        // Save the new article to localStorage, only in browser
        localStorage.setItem('selectedArticle', JSON.stringify(article));
      }
